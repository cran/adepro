...
* TRTSDT (Treatment start date = Reference Date) 
* LVDT (Last Visit date) 
* DTHDT (Death date)
* SAFFN (Safety analysis set flag)
* AESTDY (AE start day relative to reference date) 
* AEENDY (AE end day relative to reference date) 
* AESEVN (AE severity: 1-mild, 2-moderate, 3-severe)

If the ADSL dataset is uploaded as well, all required variables for ADAE but not related to adverse events (AEs) (variable name not starting with AE) need to be included in ADAE, too.
Optional variables which allow categorization of adverse events include the following:

* AETRTEMN (Treatment-emergent flag), 
* AESERN (Seriousness flag), 
* AERELN (Related to study drug flag), 
* AERELPRN (Related to protocol procedures flag), 
* AEACNN (Action taken with study treatment where AEACNN=1 means "DRUG WITHDRAWN")
